...
export function createApp(service?: AuthService, subscriptions?: SubscriptionService) {
  ...
  app.get('/v1/me/entitlement', async (req, reply) => {
    if (!subscriptionService) return reply.code(503).send({error:'SUBSCRIPTION_NOT_CONFIGURED'});
    try { const {user,deviceId}=await auth.authenticateContext(req.headers.authorization); const repo=(subscriptionService as any).subscriptions as PgSubscriptionRepository; const result=await withTransaction(async tx=>{const s=await repo.findCurrent(tx,user.id);const isPro=!!s&&PRO_STATES.has(s.status);const allowed=isPro?await (pgRepositories().devices as any).claimProSlot(tx,user.id,deviceId):false;return {s,isPro,allowed};}); const token=await subscriptionService.token({sub:user.id,device_id:deviceId,subscription_state:result.s?.status||'free',is_pro:result.isPro&&result.allowed,product_id:result.s?.product_id||null}); return {deviceId,state:result.s?.status||'free',isPro:result.isPro&&result.allowed,planDisplayName:result.isPro?'PackageHub Pro':null,productId:result.s?.product_id||null,expiresAt:result.s?.expires_at?.toISOString?.()||null,autoRenewEnabled:result.s?.auto_renew_enabled??false,deviceAccess:result.isPro&&result.allowed?'allowed':result.isPro?'limitReached':'allowed',signedEntitlementToken:token||null}; }
