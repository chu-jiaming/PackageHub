...
export function createApp(service?: AuthService) {
  ...
  const auth = service ?? new AuthService(new AppleJwtVerifier(clientId), new AppleHttpTokenExchange(config), secret, encryptionKey);
  ...
  app.get('/v1/me', async (req, reply) => { try { const u = await auth.authenticate(req.headers.authorization); return { id: u.id, displayName: u.displayName, email: u.email }; } catch (_) { return reply.code(401).send({ error: 'UNAUTHORIZED' }); } });
